#!/usr/bin/env python3
"""
Validate that rollout-policy rules can route a window set onto the preview class.
"""
...
def validate_rollout_policy_rule_routing(errors: list[str]) -> None:
    with tempfile.TemporaryDirectory(
        prefix="autotiktok-runtime-profile-rollout-policy-rule-"
    ) as temp_dir:
        ...
        rule_window_set_path = temp_root / "window-set.preview-rule.json"
        current_manifest_path = temp_root / "history-manifest.current.json"
        rule_manifest_path = temp_root / "history-manifest.preview-rule.json"
        current_batch_path = temp_root / "eval-batch.current.json"
        rule_batch_path = temp_root / "eval-batch.preview-rule.json"
    ...
    expect(
        ...
        "optimizer eval batch summary semantics drift between policy-default and policy-rule routing",
        errors,
    )
    ...
    expect(
        ...
        == "production",
        "default rollout policy path should use the production class",
        errors,
    )
    expect(
        current_window_set["generatedFrom"].get("runtimeProfileCatalogLane") == "current",
        "default rollout policy path should resolve the current catalog lane",
        errors,
    )
    expect(
        rule_window_set["generatedFrom"].get("runtimeProfileLaneSelectionSource")
        == "policy_rule",
        "rule-routed rollout path should use policy_rule",
        errors,
    )
    expect(
        rule_window_set["generatedFrom"].get("runtimeProfileRolloutClass")
        == "preview_canary",
        "rule-routed rollout path should switch to preview_canary",
        errors,
    )
    expect(
        rule_window_set["generatedFrom"].get("runtimeProfileMatchedRolloutRuleId")
        == "preview_batch_label",
        "rule-routed rollout path should report the preview_batch_label rule id",
        errors,
    )
    expect(
        rule_window_set["generatedFrom"].get("runtimeProfileCatalogLane") == "preview",
        "rule-routed rollout path should resolve the preview catalog lane",
        errors,
    )

    expect(
        rule_summary.get("runtimeProfileIds") == preview_profile_ids,
        "rule-routed rollout path should use preview canonical runtimeProfileIds",
        errors,
    )
    expect(
        rule_summary.get("runtimeProfileRequestedIds") == preview_alias_ids,
        "rule-routed rollout path should preserve legacy planner ids as requested ids",
        errors,
    )
    expect(
        rule_summary.get("runtimeProfileAliasAppliedCount")
        == len(rule_summary.get("runtimeProfileIds", [])),
        "rule-routed rollout path should apply alias resolution for every runtime profile",
        errors,
    )
...
def main() -> int:
    errors: list[str] = []
    try:
        validate_rollout_policy_rule_routing(errors)
    except Exception as exc:  # pragma: no cover
        print(f"[ERROR] unexpected failure: {exc}")
        return 1
    if errors:
        print("[ERROR] runtime profile rollout policy rule routing validation failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("Runtime profile rollout policy rule routing validation passed.")
    return 0
