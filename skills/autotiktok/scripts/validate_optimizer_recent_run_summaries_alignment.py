#!/usr/bin/env python3
"""
Validate committed optimizer recent run summaries artifact against deterministic generation.
"""
...
BUILD_RECENT_RUN_SUMMARIES_SCRIPT = (
    ROOT
    / "skills"
    / "autotiktok-strategy-optimizer"
    / "scripts"
...
def main() -> int:
    try:
        with tempfile.TemporaryDirectory(
            prefix="autotiktok-recent-run-summaries-alignment-"
        ) as temp_dir:
            ...
            if payload != committed:
                raise RuntimeError(
                    "committed optimizer recent run summaries do not match deterministic generation"
                )
            if payload["summary"]["runSummaryCount"] != 1:
                raise RuntimeError(
                    "optimizer recent run summaries should aggregate one run summary"
                )
            if payload["summary"]["jobRunCount"] != 4:
                raise RuntimeError(
                    "optimizer recent run summaries should summarize four job runs"
                )
            if payload["summary"]["errorCount"] != 0:
                raise RuntimeError(
                    "optimizer recent run summaries should summarize zero failures for the committed sample"
                )
        print(
            "Optimizer recent run summaries fixture is aligned with deterministic generation."
        )
        return 0
